...
class PolynomialRegressionDatasetOneTest {
    ...
    void degreeTwoTestCase() {
        ...
        double[] testingExpected = {43.7107685, 45.960487, 47.9868974, 50.1511668, 52.7865944, 54.8932828};
    ...
    void degreeFourTestCase() {
        ...
        model = new PolynomialRegression(4);
        model.fit(features, targets);
        double[] values = ArrayUtils.round(model.getParams().T().getValuesAsDouble()[0], 8);
        double[] predictions = ArrayUtils.round(model.predict(tests), 7);
    ...
    void degreeFiveTestCase() {
        ...
        model = new PolynomialRegression(5);
        model.fit(features, targets);
        double[] values = ArrayUtils.round(model.getParams().T().getValuesAsDouble()[0], 8);
        double[] predictions = ArrayUtils.round(model.predict(tests), 7);
